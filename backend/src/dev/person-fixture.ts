  ...
  try {
    const result = await runPersonFixture({
      env: {
        ...
        personFixtureFirstName: process.env.PERSON_FIXTURE_FIRST_NAME,
        personFixtureLastName: process.env.PERSON_FIXTURE_LAST_NAME,
      },
      prisma,
    });

    if (result === 'created') {
      console.log('controlled person fixture created');
    } else {
      console.log('controlled person fixture already exists');
    }
  } finally {
